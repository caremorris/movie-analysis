...
We find that the best model only includes 7 variables, as including extra variables only increases the error. Next, we present the coeficients of this model:
...
We try taking the logarithm of the variables votes. Now observe that there is no discernable pattern in the distribution of the errors. 
...
With this transformation we also observe an important improvements in terms of Adjusted R squared, since it increased aprox 8%, and the Test MSE error has decreased slightly to 1.037.
...
We will repeat this process taking the logarithm of the votes.
