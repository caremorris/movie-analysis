...
anova(fit.1, fit.2, fit.3, fit.4, fit.5)
```
Based on the ANOVA results, there is evidence to reject the null hypothesis that the simplest model is sufficient. Thus, the GAM with a spline for each of year, length, and budget, along with linear variables animation, comedy, and drama, is the best model.
...
We conclude that taking the log of the votes does improve the GAM; however, it provided more of benefit in our linear models.

<!-- only print test MSE, not summary of all the models -->
