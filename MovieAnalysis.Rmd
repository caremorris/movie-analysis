...
(2) On the *length* column, we can see a cluster of short films on the right hand side of the plots. They clearly have a different behavior than full-feature ones, so we will exclude them from the analysis.
...
After proceding with these changes, we identified one extreme outlier in terms of nominal budget: "Voyna i mir", or "War & Peace" (1996). 

```{r Voyna i mir, fig.width=6, fig.height=3}
qplot(movies$adjBudget)
...
#plot TEST MSE vs number of covariates
...
min(mse.test.errors)
summary(lm.fit)
#The residuals exhibit a clear inverted U-shape, which provides a strong indication of non-linearity in the data
plot(lm.fit)

#fit logarithm of votes
lm.fit.log = lm(rating ~ year + length + I(log(votes)) + Action + Animation + Comedy
            + Drama + Documentary + adjBudget, data = train.set)
#there is no discernable pattern in the errors
plot(lm.fit.log)
fit.log.predic = predict(lm.fit.log, newdata = test.set[, !names(test.set) %in% c('rating')])
#improved adjusted R squared and TEST MSE
summary(lm.fit.log)
