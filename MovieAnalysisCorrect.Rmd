...
The first graph is produced by fitting a polynomial of degree 3, while the graph on the rigth is produced fitting a regular line. In both graphs, the blue line represents the fitted value from both models and the red lines an estimated 95% confidence interval.
...
The plot of our GAM displays a plot for each variable, showing its individual contribution to the additive model. The standard errors are repressented by the dotted lines.
...
We conclude that the GAM with smoothing splines is the best model for predicting the rating of a movie. Furthermore, taking the logarithm of the votes provides a slight improvement to the accuracy of the model; however, the benefit of the logarithm was more substatial in our linear models.
